...
# ==========================================
# PHẦN 1: KẾT NỐI & TỰ ĐỘNG VÁ LỖI DATABASE (XỬ LÝ TRIỆT ĐỂ)
# ==========================================
def init_db():
    conn = sqlite3.connect('exam_db.sqlite')
    c = conn.cursor()
    
    # 1. Tạo bảng classes nếu chưa có
    c.execute('''CREATE TABLE IF NOT EXISTS classes
                 (class_id INTEGER PRIMARY KEY AUTOINCREMENT, 
                  class_name TEXT UNIQUE, 
                  teacher_name TEXT DEFAULT 'Chưa phân công')''')
    
    # Kiểm tra và thêm cột teacher_name cho bảng classes
    ...
    # Kiểm tra và thêm cột cho bảng users (Tránh lỗi OperationalError)
    ...
    # 3. Tạo bảng results nếu chưa có
...
def main():
    ...
    if st.session_state.role == 'admin':
        ...
        with t2:
            ...
            conn.close()
            if not df_cls.empty:
                del_cls = st.selectbox("Chọn lớp muốn xóa", df_cls['class_name'], key="del_cls_sb")
        ...
        with t3:
            ...
            with col_b:
                ...
                if not df_hs.empty:
                    ...
                    st.write("---")
                    del_user = st.selectbox("Chọn tài khoản để xóa", df_hs['Tên đăng nhập'], key="del_user_sb")
