...
def init_db():
    ...
    # Vá lỗi thiếu cột teacher_name nếu có
    ...
    # Vá lỗi thiếu cột class_name nếu có
...
class ExamGenerator:
    ...
    def generate_all(self):
        ...
        self.build_q(8, "Đỉnh Parabol $y = x^2$ là:", "(0; 0)", ["(1; 1)", "(0; 1)", "(1; 0)"], "HD: Đỉnh tại gốc tọa độ O.")
        self.build_q(9, "Đồ thị hàm số $y = ax^2$ gọi là gì?", "Parabol", ["Đường thẳng", "Đường tròn", "Elip"], "HD: Lý thuyết căn bản.")

        # 3. PHƯƠNG TRÌNH (8 câu)
        self.build_q(10, r"Hệ $\begin{cases} x+y=3 \\ x-y=1 \end{cases}$ có nghiệm:", "(2; 1)", ["(1; 2)", "(2; 2)", "(3; 0)"], "HD: Giải bằng cộng đại số.")
        for i in range(11, 18):
            self.build_q(i, rf"Biệt thức $\Delta$ của $x^2 - {i}x + 1 = 0$:", rf"${i**2 - 4}$", [rf"${i**2 + 4}$", "0", "4"], "HD: $\Delta = b^2 - 4ac$.")

        # 4. BẤT PHƯƠNG TRÌNH (3 câu)
        self.build_q(18, "Nghiệm của $2x - 8 > 0$:", "x > 4", ["x < 4", "x > -4", "x < -4"], "HD: $2x > 8 \Rightarrow x > 4$.")
        self.build_q(19, "Số nguyên lớn nhất thỏa $x < 3.5$:", "3", ["4", "2", "3.5"], "HD: Số nguyên nhỏ hơn 3.5.")
        self.build_q(20, "Điều kiện để $mx+1>0$ là BPT bậc nhất:", "m ≠ 0", ["m = 0", "m > 0", "m < 0"], "HD: Hệ số a khác 0.")
        ...
        # 6. ĐƯỜNG TRÒN (6 câu)
        self.build_q(26, "Góc nội tiếp chắn nửa đường tròn bằng:", "90°", ["180°", "45°", "60°"], "HD: Luôn bằng 90 độ.")
        for i in range(27, 32):
            self.build_q(i, rf"Chu vi đường tròn $R = {i}$:", rf"{2*i}π", [f"{i}π", f"{i**2}π", "20"], "HD: $C = 2\pi R$.")

        # 7. HÌNH KHỐI (3 câu)
        self.build_q(32, "Thể tích hình cầu bán kính R:", "4/3 π R³", ["4 π R²", "π R² h", "1/3 π R² h"], "HD: Công thức SGK.")
        self.build_q(33, "S xung quanh hình trụ:", "2πrh", ["πrh", "πr²h", "2πr"], "HD: Chu vi đáy nhân cao.")
        self.build_q(34, "Hình nón r=3, h=4, đường sinh l:", "5", ["7", "1", "25"], "HD: $l = \sqrt{r^2 + h^2}$.")

        # 8. THỐNG KÊ (6 câu)
        img37 = generate_histogram_base64([10, 20, 40, 20, 10])
        self.build_q(35, "Xác suất biến cố chắc chắn:", "1", ["0", "0.5", "100"], "HD: P luôn bằng 1.")
        self.build_q(37, "Nhóm tỉ lệ cao nhất (40%) là:", "160-170", ["140-150", "150-160", "170-180"], "HD: Xem cột cao nhất.", img_b64=img37)
...
def main():
    ...
    if st.session_state.role == 'admin':
        ...
        with t3:
            ...
            with col_b:
                st.markdown("**📂 Xuất dữ liệu học sinh**")
                conn = sqlite3.connect('exam_db.sqlite')
                ...
                conn.close()
                if not df_hs.empty:
                    # Nút xuất Excel (yêu cầu openpyxl/xlsxwriter)
                    try:
                        out_ex = BytesIO()
                        ...
                        # Backup nếu thiếu thư viện Excel
                        csv = df_hs.to_csv(index=False).encode('utf-8-sig')
                        st.download_button("📥 Tải File CSV (Thay thế Excel)", csv, "ds_taikhoan.csv", "text/csv")
                else: st.info("Chưa có học sinh nào.")
